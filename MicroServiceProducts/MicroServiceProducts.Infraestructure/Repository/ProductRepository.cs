...
namespace MicroServiceProduct.Infraestructure.Repository
{
    public class ProductRepository : IProductRepository
    {
        ...
        public Product? Read(Guid id)
        {
            using var conn = _database.GetConnection();
            using var cmd = conn.CreateCommand();
            // Select explicit columns instead of p.* to avoid name collisions and ensure category_name is present
            cmd.CommandText = @"SELECT p.id, p.name, p.description, p.category_id, c.name AS category_name, p.price, p.stock, p.created_at
                                FROM products p
        ...
        public List<Product> GetAll()
        {
            ...
            using var conn = _database.GetConnection();
            using var cmd = conn.CreateCommand();
            // Explicitly select columns and alias category name to avoid ambiguity
            cmd.CommandText = @"SELECT p.id, p.name, p.description, p.category_id, c.name AS category_name, p.price, p.stock, p.created_at
                                FROM products p
                                LEFT JOIN categories c ON p.category_id = c.id
